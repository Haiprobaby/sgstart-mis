...
class AcademicController extends Controller
{
    ...
    public function ajaxSubjectDropdown(Request $request)
    {
        ...
        try {
            ...
            if (Auth::user()->role_id=='1') {
                ...
                $class_id = $request->class;
                $allSubjects = SmAssignSubject::where([['section_id', '=', $request->id], ['class_id', $class_id], ['teacher_id', $staff_info->id]])->get();
    ...
    public function classReportSearch(Request $request)
    {
        ...
        try {
            ...
            $assign_subjects = $assign_subjects->where('created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->get();
            ...
            foreach ($students as $student) {
                $fees_assigns = SmFeesAssign::where("student_id", $student->id)->where('active_status', 1)->where('created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->get();
                foreach ($fees_assigns as $fees_assign) {
                    $fees_masters = SmFeesMaster::where('id', $fees_assign->fees_master_id)->where('created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->get();
