...
class SmParentPanelController extends Controller
{
    ...
    public function myChildren($id)
    {
        ...
        try {
            ...
            $timelines = SmStudentTimeline::where('staff_student_id', $student_detail->id)->where('type', 'stu')->where('created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->where('visible_to_student', 1)->get();
    ...
    public function onlineExamination($id)
    {
        ...
        try {
            ...
            $online_exams = SmOnlineExam::where('active_status', 1)->where('status', 1)->where('class_id', $student->class_id)->where('section_id', $student->section_id)->where('end_time', '>', $now)->where('created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->get();
            // return $online_exams;
            $marks_assigned = [];
    ...
    function leaveApply(Request $request)
    {
        try {
            $user = Auth::user();
            if ($user) {
                $my_leaves = SmLeaveDefine::where('role_id', 2)->get();
                $apply_leaves = SmLeaveRequest::where('role_id', 2)->where('active_status', 1)->where('created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->get();
                $leave_types = SmLeaveDefine::where('role_id', 2)->where('active_status', 1)->where('created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->get();
            } else {
    ...
    public function parentBookIssue()
    {
        try {
            ...
            $subjects = SmSubject::select('id', 'subject_name')->where('active_status', 1)->where('created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->get();
    ...
    public function dormitory($id)
    {
        try {
            $student_detail = SmStudent::where('id', $id)->first();
            // return $student_detail;
            $room_lists = SmRoomList::where('active_status', 1)->where('id', $student_detail->room_id)->get();
            $room_lists = $room_lists->groupBy('dormitory_id');
            $room_types = SmRoomType::where('active_status', 1)->get();
            $dormitory_lists = SmDormitoryList::where('active_status', 1)->where('id', $student_detail->dormitory_id)->get();
    ...
    public function homework($id)
    {
        try {
            $student_detail = SmStudent::where('id', $id)->first();
            $homeworkLists = SmHomework::where('class_id', $student_detail->class_id)->where('section_id', $student_detail->section_id)->where('created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->get();
