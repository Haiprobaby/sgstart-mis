...
class SmCommunicateController extends Controller
{
    ...
    public function studStaffByRole(Request $request)
    {
        ...
        try {
            if ($request->id == 2) {
                $allStudents = SmStudent::where('active_status', '=', 1)->where('created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->get();
                $students = [];
    ...
    {
        try {
            $emailSmsLogs = SmEmailSmsLog::where('created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->orderBy('id', 'DESC')->get();
