...
class SmFeesMasterController extends Controller
{
    ...
    public function feesAssignSearch(Request $request)
    {
        ...
        try{
            $classes = DB::table('sm_classes')->where('active_status', 1)->where('created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->get();
            $genders = DB::table('sm_base_setups')->where('active_status', '=', '1')->where('base_group_id', '=', '1')->get();
            $categories = DB::table('sm_student_categories')->get();
