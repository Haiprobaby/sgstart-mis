...
// Phạm Trọng hải
    ...
    {
        $list = sm_fees_group::all();  ///lấy danh sách các group 
        return view('frontEnd.home.admissions',['list'=>$list]); //gửi danh sách vè view 
    }
// Phạm Trọng hải
    public function getdetails($id)
    {

        $group=sm_fees_group::find($id); //tìm group theo id
        $years = years::all()->where('id_group',$id); //tìm năm học theo id group
        $fees = sm_fees::all()->where('fees_group',$id); //lấy tất cả phí theo id group
        $bus = bus_fee::all(); //lấy tất cả phí xe buýt
        $discount=discount::all(); //lấy tất cả discount
        $late_enrolment=late_enrolment::all(); //lấy tất cả phí ghi danh muộn
        $grades=school_grades::all(); //lấy tất cả lớp học
        $withdraw=withdraw::all(); //lấy tát cả chính sách rút học phí
        return view('frontEnd.home.admissions_details',compact('group','years','fees','bus','discount','late_enrolment','grades','withdraw'));

        ///gửi tất cả về view "admission_details"
    }
