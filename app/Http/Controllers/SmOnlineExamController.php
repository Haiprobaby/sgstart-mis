...
class SmOnlineExamController extends Controller
{
    ...
    public function index()
    {
        ...
        try{
            $online_exams = SmOnlineExam::where('status', '!=', 2)->where('created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->get();
    ...
    public function store(Request $request)
    {
        ...
        try{
            ...
            if ($result) {
                $data = SmStudent::where('class_id', $request->class)->where('section_id', $request->section)->where('created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->get();
                if (!empty($data)) {
    ...
    public function manageOnlineExamQuestion($id)
    {
        ...
        try{
            $online_exam = SmOnlineExam::find($id);
            $question_banks = SmQuestionBank::where('class_id', $online_exam->class_id)->where('section_id', $online_exam->section_id)->where('created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->get();
    
            $assigned_questions = SmOnlineExamQuestionAssign::where('online_exam_id', $id)->where('created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->get();
            $already_assigned = [];
    ...
    public function onlineExamPublish($id)
    {
        ...
        try{
            ...
            $students = SmStudent::select('id')->where('class_id', $class_id)->where('section_id', $section_id)->where('created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->get();
    ...
    public function onlineExamMarksRegister($id)
    {
        ...
        try{
            ...
            $students = SmStudent::where('class_id', $online_exam_question->class_id)->where('section_id', $online_exam_question->section_id)->where('created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->get();    
            $present_students = [];
            foreach ($students as $student) {
