...
class SmOptionalSubjectAssignController extends Controller
{
    ...
    public function index(Request $request)
    {
        // return $request;
        try {
            $classes = SmClass::join('sm_class_optional_subject','sm_class_optional_subject.class_id','=','sm_classes.id')->where('sm_classes.created_at', 'LIKE', '%' . YearCheck::getYear() . '%')
            ->select('sm_classes.*')
            ->get();        
            $sections = SmClassSection::all();
            $assign_subjects = SmAssignSubject::where('class_id', 1)->where('section_id', 1)->get(); 
    ...
    public function assignOptionalSubjectSearch(Request $request)
    {
        ...
        try{
            ...
            $classes = SmClass::join('sm_class_optional_subject','sm_class_optional_subject.class_id','=','sm_classes.id')->where('sm_classes.created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->select('sm_classes.*')->get(); 
            //return $classes;
