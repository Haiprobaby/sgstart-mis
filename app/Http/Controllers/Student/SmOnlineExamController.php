...
class SmOnlineExamController extends Controller
{
	...
	public function studentOnlineExam()
	{
		try {
			...
			$online_exams = SmOnlineExam::where('active_status', 1)->where('status', 1)->where('class_id', $student->class_id)->where('section_id', $student->section_id)->where('end_time', '>', $now)->get();
			$marks_assigned = [];
	...
	{
		try {
			$result_views = SmStudentTakeOnlineExam::where('active_status', 1)->where('status', 2)->where('student_id', @Auth::user()->student->id)->get();
