(function ($) {
                ...
                success: function (data) {
                    ...
                    $.each(data, function (i, item) {
                        if (item.length) {
                            ...
                            $.each(item, function (i, section) {
                                $('#c_select_section').append($('<option>', {
                                    value: section.id,
                                    text: section.section_name
                                }));

                                $("#c_select_section_div ul").append("<li data-value='" + section.id + "' class='option'>" + section.section_name + "</li>");
                            });
                        } else {
        ...
        if ($('#marks_register_store').length) {
            ...
            $('form#marks_register_store').on('submit', function (event) {
                ...
                $('.marks_input').each(function () {
                    ...
                    if(parseInt($(this).siblings('#part_marks').val()) < parseInt($(this).val())){
                        i++;
                    }
                    if($(this).val() == ""){
                        j++;
                ...
                if(i > 0){
                    setTimeout(function () {
                        toastr.error('Obtained mark can not be higher than distributed mark.!', 'Error Alert', {
                            timeOut: 5000
                        });
                    }, 500);
                    return false;
    ...
    $(document).on("click", "#fees_groups_invoice_print_button", function (event) {
        var url = $('#url').val();
        var student_id = $('#student_id').val();
        ...
        if (sList != "") {
            $("#fees_groups_invoice_print_button").attr("href", url + "/fees-payment-invoice-print/" + sList + "/" + student_id);
...
// student id card print 
    ...
    $(document).on("click", ".generate-id-card-print-all", function (event) {
        ...
        if (len.length > 8) {
            toastr.warning('You can not select more than 8 student!');
            $("#genearte-id-card-print-button").attr("href", 'javascript:;');
            $("#genearte-id-card-print-button").removeAttr("target");console.log('p');
    ...
    $("#admission-query-store").on("submit", function () {
        ...
        if ($('#admission-query-store #no_of_child').val() == "") {
            count++;
            $('#admission-query-store #no_of_childError').html('Number of child is required');
        } else {
                ...
                success: function (data) {
                    ...
                    $.each(data, function (i, item) {
                        ...
                        if (item.length) {
                            ...
                            $.each(item, function (i, section) {
                                $('#select_fees_type').append($('<option>', {
                                    value: section.assign_id,
                                    text: section.fees_type
                                }));

                                $("#select_fees_type_div ul").append("<li data-value='" + section.assign_id + "' class='option'>" + section.fees_type + "</li>");
                            });
                        } else {
